...
<aside :class="{'translate-x-0': sidebarOpen, '-translate-x-[150%]': !sidebarOpen}"
  ...
  <nav class="flex-1 px-3 py-6 space-y-1 overflow-y-auto custom-scrollbar relative z-0 pr-2">
    ...
    <a href="{{ route('admin.laporan.panitia') }}" wire:navigate
      ...
      <svg class="w-5 h-5 {{ request()->routeIs('admin.laporan.panitia') ? 'text-primary-400' : 'text-primary-300' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <a href="{{ route('admin.laporan.mudhohi') }}" wire:navigate
      ...
      <svg class="w-5 h-5 {{ request()->routeIs('admin.laporan.mudhohi') ? 'text-primary-400' : 'text-primary-300' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <a href="{{ route('admin.laporan.mustahiq') }}" wire:navigate
      ...
      <svg class="w-5 h-5 {{ request()->routeIs('admin.laporan.mustahiq') ? 'text-primary-400' : 'text-primary-300' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z"/>
      </svg>
      <span class="font-medium text-sm">Laporan Mustahiq</span>
    </a>
    <a href="{{ route('admin.laporan.rab') }}" wire:navigate
      class="flex items-center gap-3 px-4 py-2.5 rounded-2xl transition {{ request()->routeIs('admin.laporan.rab') ? 'bg-white/10 text-primary-50 shadow-inner border border-white/5 backdrop-blur-sm' : 'hover:bg-white/5 text-primary-100 hover:text-white' }}">
      <svg class="w-5 h-5 {{ request()->routeIs('admin.laporan.rab') ? 'text-primary-400' : 'text-primary-300' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
      </svg>
      <span class="font-medium text-sm">Laporan Data RAB</span>
    </a>
    ...
    <div class="pt-6 mt-4 border-t border-white/10">
      ...
      <a href="{{ route('admin.users') }}" wire:navigate
        class="flex justify-between items-center px-4 py-3 rounded-2xl transition {{ request()->routeIs('admin.users') ? 'bg-white/10 text-primary-50 shadow-inner border border-white/5 backdrop-blur-sm' : 'hover:bg-white/5 text-primary-100 hover:text-white' }}">
        <div class="flex items-center gap-3">
          <svg class="w-5 h-5 {{ request()->routeIs('admin.users') ? 'text-primary-400' : 'text-primary-300' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"/>
          </svg>
          <span class="font-medium text-sm">Manajemen User</span>
        </div>
        <span class="text-[9px] {{ request()->routeIs('admin.users') ? 'bg-amber-400 text-amber-900' : 'bg-amber-500/20 text-amber-400' }} font-bold px-2 py-1 rounded-lg">ADMIN</span>
      </a>
